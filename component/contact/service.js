...
const postPreviousContact = async(req, res) => {
    try {
        ...
        if (req.params.key==='none' && req.params.sortby === 'none'){
            param +='';
        } else if (req.params.key && req.params.sortby == 'none') {
            param += '&key=' + req.params.key;
        } else if (req.params.key == 'none' && req.params.sortby) {
            param += '&sortby=' + req.params.sortby;
        } else if (req.params.key && req.params.sortby) {
            param += '&key=' + req.params.key + '&sortby=' + req.params.sortby;
        }
        res.redirect('/contact/list?page=' + page + param);
    } catch (e) {
        console.log(e);
        res.render('pages/error/500');
    }
